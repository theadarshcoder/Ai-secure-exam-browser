 ...
 * 
 * Shared computation engine for Student Intelligence analytics.
 * Used by BOTH the controller (sync on cache miss) and the worker (async cache warmer).
 * 
 * SINGLE SOURCE OF TRUTH — eliminates duplicate aggregation logic.
 */
...
// --- DTO: Guarantee response shape for frontend contract ---
const buildStudentInfo = (user) => ({
    _id: user._id,
    name: user.name,
    email: user.email,
    profilePicture: user.profilePicture || null,
    isVerified: !!user.isVerified,
    verificationIssue: user.verificationIssue || (user.isVerified ? null : 'Identity not verified')
});

/**
 * Compute the full intelligence report for a student.
 * @param {string} studentId - MongoDB ObjectId string
 * @param {number} page - Pagination page (default 1)
 * @param {number} limit - Items per page (default 10)
 * @returns {Object} Complete intelligence response object
 */
const computeStudentIntelligence = async (studentId, page = 1, limit = 10) => {
    ...
    const student = await User.findById(studentId).select('name email profilePicture isVerified verificationIssue').lean();
    if (!student) {
...
module.exports = { computeStudentIntelligence, buildStudentInfo };
